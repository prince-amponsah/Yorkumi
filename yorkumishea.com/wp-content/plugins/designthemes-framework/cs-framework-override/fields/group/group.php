...
class CSFramework_Option_group extends CSFramework_Options {
  ...
  public function output() {
    ...
    $acc_title   = ( isset( $this->field['accordion_title'] ) ) ? $this->field['accordion_title'] : esc_html__( 'Adding', 'designthemes-framework' );
    ...
    $dt_default_type    = ( isset( $this->field['dt_default_type'] ) && !empty( $this->field['dt_default_type'] ) ) ? true : false;
    ...
    echo '<div class="cs-group cs-group-'. $el_class .'-adding hidden">';
      ...
      if( !$dt_default_type ) {
        echo '<div class="cs-element cs-text-right cs-remove"><a href="#" class="button cs-warning-primary cs-remove-group">'. esc_html__( 'Remove', 'designthemes-framework' ) .'</a></div>';
      }
      echo '</div>';
