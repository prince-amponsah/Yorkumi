...
if( ! function_exists( 'cs_get_path_locate' ) ) {
  function cs_get_path_locate() {
    ...
    $plugin_dir     = wp_normalize_path( WP_PLUGIN_DIR . '\designthemes-framework' );
    ...
    $directory_uri  = ( $located_plugin ) ? WP_PLUGIN_URL . '/designthemes-framework' : get_template_directory_uri();
...
if( ! function_exists( 'cs_locate_template' ) ) {
  function cs_locate_template( $template_name ) {
    ...
    $dir_plugin   = wp_normalize_path( WP_PLUGIN_DIR . '\designthemes-framework' );
 ...
 *
 */
load_textdomain( 'designthemes-framework', CS_DIR .'/languages/'. cs_get_locale() .'.mo' );