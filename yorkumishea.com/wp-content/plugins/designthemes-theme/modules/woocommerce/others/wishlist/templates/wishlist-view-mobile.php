...
<ul class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo esc_attr($show_cb) ? 'with-checkbox' : ''; ?>" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
	...
	if ( $wishlist && $wishlist->has_items() ) :
					...
					<div class="item-wrapper">
						<div class="product-thumbnail">
							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
								<?php echo savon_html_output($product->get_image()); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</a>
						</div>
							...
							<?php if ( $show_variation || $show_dateadded || $show_price ): ?>
								<table class="item-details-table">
									...
									<?php if ( $show_dateadded && $item->get_date_added() ): ?>
										<tr>
											<td class="label">
												<?php esc_html_e( 'Added on:', 'designthemes-theme' ); ?>
											</td>
											<td class="value">
									...
									<?php if ( $show_price || $show_price_variations ) : ?>
										<tr>
											<td class="label">
												<?php esc_html_e( 'Price:', 'designthemes-theme' ); ?>
											</td>
											<td class="value">
												<?php
												if ( $show_price ) {
													echo savon_html_output($item->get_formatted_product_price()); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
												}

												if ( $show_price_variations ) {
													echo savon_html_output($item->get_price_variation()); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
												}
												?>
											</td>
										</tr>
					...
					<div class="additional-info-wrapper">
						<?php if ( $show_quantity || $show_stock_status ): ?>
							<table class="additional-info">
								<?php if ( $show_quantity ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Quantity:', 'designthemes-theme' ); ?>
										</td>
										<td class="value">
								...
								<?php if ( $show_stock_status ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Stock:', 'designthemes-theme' ) ?>
										</td>
										<td class="value">
											<?php echo esc_attr($stock_status) === 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'designthemes-theme' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'designthemes-theme' ) . '</span>'; ?>
										</td>
									</tr>
								<?php endif ?>
							</table>
							...
							<div class="move-to-another-wishlist">
								<?php if ( 'select' == $move_to_another_wishlist_type ): ?>
									<select class="change-wishlist selectBox">
										<option value=""><?php esc_html_e( 'Move', 'designthemes-theme' ); ?></option>
										<?php
								...
								<?php else: ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										<?php echo esc_html( apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'designthemes-theme' ) ) ); ?>
									</a>
								<?php endif; ?>
							</div>
						<?php endif; ?>
						...
						<?php if ( $show_remove_product ): ?>
							<div class="product-remove">
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'designthemes-theme' ) ) ); ?>"><i class="fa fa-trash"></i></a>
							</div>
						<?php endif; ?>
					</div>
				</li>
		...
		<p class="wishlist-empty">
			<?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'designthemes-theme' ) ) ); ?>
		</p>
	<?php
	endif;
	...
	if ( ! empty( $page_links ) ) : ?>
		<p class="pagination-row">
			<?php echo savon_html_output($page_links); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</p>
	<?php endif ?>
